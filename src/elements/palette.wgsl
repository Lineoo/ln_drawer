...
@fragment
fn palette_main(in: VertexOutput) -> @location(0) vec4f {
    return vec4f(hsl_to_rgb(vec3f(palette.h, in.uv.x, in.uv.y)), 1.0);
}

@fragment
fn palette_hue(in: VertexOutput) -> @location(0) vec4f {
    return vec4f(hsl_to_rgb(vec3f(in.uv.x, palette.h, palette.l)), 1.0);
}
...
fn hsv_to_rgb(hsv: vec3f) -> vec3f {
    return hsv.z * mix(vec3f(1.0), hue_to_rgb(hsv.x), hsv.y);
}
