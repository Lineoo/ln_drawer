...
impl Panel {
    pub fn build(self, world: &World) -> Handle<Panel> {
        ...
        let panel = world.insert(self);

        world.observer(panel, move |&SetWidgetRectangle(rect), world| {
            ...
            collider.rect = rect;
            world.queue_trigger(panel, WidgetRectangle(rect));
        });

        world.observer(panel, move |&SetWidgetVisible(enabled), world| {
            let mut this = world.fetch_mut(panel).unwrap();
            ...
            world.queue_trigger(panel, WidgetVisible(enabled));
        });

        world.observer(panel, move |anim: &SetPanelAnimation, world| {
            let mut this = world.fetch_mut(panel).unwrap();
            let mut back_rect_anim = world.fetch_mut(back_rect_anim).unwrap();
            this.rect = anim.dst;
