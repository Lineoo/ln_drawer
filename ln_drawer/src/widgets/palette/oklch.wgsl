...
fn color_main_palette(uv: vec2f) -> vec4f {
    ...
    let color = oklch_to_linear_srgb(vec3f(uv.yx * vec2f(1, 0.4), palette.oklch.z));
...
fn color_hue_band(radius: f32, angle: f32) -> vec4f {
    ...
    let color = oklch_to_linear_srgb(vec3f(palette.oklch.xy, angle));
    return select(vec4f(), vec4f(color, 1) * alpha, all(color <= vec3f(1) & color >= vec3f(0)));
}

fn color_main_knob(uv: vec2f) -> vec4f {
    let diff = distance(uv, palette.oklch.yx / vec2f(0.4, 1)) - palette.main_knob_size;
    let width = fwidth(diff) * 0.5;
    if diff < 0.010 {
        let factor = smoothstep(-width, width, diff - 0.008);
        let color = oklch_to_linear_srgb(palette.oklch.xyz);
        return mix(vec4f(color, 1), WHITE, factor);
    } else if diff < 0.014 {
...
fn color_hue_knob(radius: f32, angle: f32) -> vec4f {
    ...
    let diff_d = abs(fract(palette.oklch.z / TAU) - fract(angle / TAU));
    ...
    if diff < 0.0010 {
        ...
        let color = oklch_to_linear_srgb(palette.oklch.xyz);
        return mix(vec4f(color, 1), WHITE, factor) * alpha;
    } else if diff < 0.0020 {
