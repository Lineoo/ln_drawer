...
fn cs_main(@builtin(global_invocation_id) id: vec3u) {
    ...
    let dst = demul_alpha(srgb_gamma_decode(textureLoad(destination_texture, dst_coords)));
    var dst_oklab = mul_alpha(vec4f(linear_srgb_to_oklab(dst.xyz), dst.a));
    for (var i = 0u; i < draws_length; i++) {
        ...
        let src_oklab = mul_alpha(vec4f(linear_srgb_to_oklab(srgb_gamma_decode(draw.color).xyz), draw.color.a)) * draw.flow.a * mask;

        let over_pre = src_oklab + dst_oklab * (1. - src_oklab.a);
        let under_pre = dst_oklab + src_oklab * (1. - dst_oklab.a);

        dst_oklab = mix(under_pre, over_pre, draw.flow);
    }

    textureStore(swap_texture, swp_coords, srgb_gamma_encode(mul_alpha(vec4f(oklab_to_linear_srgb(demul_alpha(dst_oklab).xyz), dst_oklab.a))));
}
